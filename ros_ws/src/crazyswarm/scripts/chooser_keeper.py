...
if __name__ == '__main__':
	...
	if not os.path.exists(os.path.join(args.configpath, "allCrazyfliesKeeper.yaml")) or \
		not os.path.exists(os.path.join(args.configpath, "crazyflieTypes.yaml")) or \
		not os.path.exists(os.path.join(args.configpath, "crazyflies_keeper.yaml")):
	...
	def save():
		nodes = selected_cfs()
		with open(os.path.join(args.configpath, "crazyflies_keeper.yaml"), 'w') as outfile:
			yaml.dump({"crazyflies": nodes}, outfile)

	allCrazyflies = read_by_id(os.path.join(args.configpath, "allCrazyfliesKeeper.yaml"))
	enabled = read_by_id(os.path.join(args.configpath, "crazyflies_keeper.yaml")).keys()
